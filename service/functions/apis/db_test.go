...
func TestReadEventsForUser(t *testing.T) {
    ...
    fakeEvent, err := CreateFakeEvent(db, GetFakeEvent()); if err != nil {
        t.Error(err)
    }
    ...
    eventsDbWithAnotherUser, err := GetEventsForUser(db, userId);
    if err != nil {
        t.Error(err)
    }
    ...
    hostUserDbEvents, err := GetEventsForUser(db, hostUserId);
    if err != nil {
        t.Error(err)
    }
