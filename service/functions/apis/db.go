...
func GetUserByAuth0Id(db *sql.DB, auth0Id string) (User, error) {
    ...
    dietaryRestrictionsArray := strings.Split(dietary_restrictions, "+")
...
func GetEvent(db *sql.DB, eventId int64) (Event, error) {
    ...
    row := db.QueryRow(`SELECT event_id, title, description,
    ...
    scanErr := row.Scan(&event_id, &title, &description,
    &happeningAt, &hostId)
...
func UpdateEvent(db *sql.DB, event Event) (Event, error) {
    ...
    if event.Description != "" {
        ...
        updates = append(updates, event.Description)
    }
    ...
    err := row.Scan(&title, &description, &happeningAt, &hostId)
    if err != nil {
        return Event{}, err
    }
...
func GetEventsForUser(db *sql.DB, userId int64) (Events, error) {
	rows, err := db.Query(
	       `(SELECT
                        events.event_id,
                        events.title,
